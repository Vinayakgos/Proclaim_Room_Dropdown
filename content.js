...
fetch(chrome.runtime.getURL('medicines.json'))
    ...
    .then(medicines => {
        ...
        function showDropdown(inputField, query = '') {
            // Exclude input fields with specific placeholders (if necessary)
            if (inputField.placeholder === 'enter amount') {
                console.log('Input field with placeholder "enter amount" detected, skipping dropdown.');
                return;
            }

            // Update this selector to match the input fields on the new website
            if (!inputField.matches('input.form-control[placeholder="Enter name"]')) {
                return; // Exit if input field doesn't match the new selector
            }
        ...
        document.addEventListener('click', function(event) {
            ...
            const isInputField = target.matches('input.form-control[placeholder="Enter name"]');
